...
export default function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="card-apex p-8 text-center">
        ...
        <h3 className="mt-4 font-display font-semibold text-cream text-2xl">
          Message sent!
        </h3>
        <p className="mt-2 text-sm text-muted-foreground">
          We'll get back to you. #TAGonTop
        </p>
        <button
          onClick={() => setSent(false)}
          className="mt-6 text-sm text-primary hover:text-primary-light"
        >
          Send another →
        </button>
      </div>
  ...
  return (
    <form
      ...
      className="card-apex p-6 sm:p-8 space-y-5"
    >
...
const inputCls =
  "w-full rounded-md bg-surface-2 border border-border px-4 py-2.5 text-sm text-cream placeholder:text-muted-strong focus:border-primary outline-none transition-colors";
...
function Field({
  ...
  return (
    <div>
      <label className="text-[11px] font-medium uppercase tracking-[0.15em] text-primary">
        {label}
      </label>
