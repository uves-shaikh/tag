    ...
    <>
      <article className="pt-28">
        ...
        <div className="container-apex max-w-3xl mt-6">
          ...
          <h1 className="mt-4 font-display font-bold text-cream text-4xl md:text-5xl leading-tight">
            {article.title}
          </h1>
          <p className="mt-4 text-sm text-muted-strong">
            {article.date} · TAG Staff
          </p>
        </div>
        ...
        <div className="container-apex max-w-3xl mt-10 prose-invert">
          {article.content.split("\n").map((p, i) => (
            <p
              key={i}
              className="text-cream-muted leading-[1.85] mb-5 whitespace-pre-line"
            >
              {p}
            </p>
          ))}
          <div className="mt-6 flex flex-wrap gap-1.5">
            {article.tags.map((t) => (
              <span key={t} className="pill-muted">
                {t}
              </span>
            ))}
          </div>
      ...
      <section className="container-apex py-20">
        ...
        <h2 className="font-display font-bold text-cream text-2xl md:text-3xl">
          Related stories
        </h2>
